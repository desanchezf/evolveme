...
class CardioSessionForm(forms.ModelForm):
    class Meta:
        model = CardioSession
        fields = [
            "user", "exercise", "date", "session_start", "session_end",
        ...
        widgets = {
            ...
            "avg_speed": forms.NumberInput(attrs={"class": "form-control", "step": "0.01"}),
...
class TrainingSessionModelForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "user" in self.fields:
            ...
            layout_rows.append(
                Row(Column("routine", css_class="col-12"), css_class="mb-3")
            )
        layout_rows.extend([
